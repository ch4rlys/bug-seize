...
switch (_code) do {
	...
	//Shift + J : Steal keys of proximity vehicules
		case 36:
		{
			if (_shift && !_alt && !_ctrlKey) then
			{
				if ( !isNull cursorTarget && (cursorTarget isKindOf "Man") && isPlayer cursorTarget && alive cursorTarget && (cursorTarget distance player < 4) && speed cursorTarget < 1 ) then
				{
					...
					{
						[[player], "life_fnc_keyThief",vehicle cursorTarget,false] spawn life_fnc_MP;
		...
		case 19: {
			...
			if(_shift && playerSide == west && {!isNull cursorTarget} && {cursorTarget isKindOf "Man"} && {(isPlayer cursorTarget)} && {(side cursorTarget in [civilian,independent,west])} && {alive cursorTarget} && {cursorTarget distance player < 3.5} && {!(cursorTarget GVAR "Escorting")} && {!(cursorTarget GVAR "restrained")} && {speed cursorTarget < 1}) then {
				if(_shift && playerSide == east && {!isNull cursorTarget} && {cursorTarget isKindOf "Man"} && {(isPlayer cursorTarget)} && {(side cursorTarget in [civilian,East,west])} && {alive cursorTarget} && {cursorTarget distance player < 3.5} && {!(cursorTarget GVAR "Escorting")} && {!(cursorTarget GVAR "restrained")} && {speed cursorTarget < 1}) then {
